...
/*   Updated: 2024/04/18 16:36:56 by tlay             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	animations(t_vars *vars)
{
	...
	y = find_x(&vars->map, 'E') * vars->img.width;
	x = find_y(&vars->map, 'E') * vars->img.height;
	print_mov(vars);
	if (vars->map.player.position_x == find_x(&vars->map, 'E')
		&& vars->map.player.position_y == find_y(&vars->map, 'E'))
	{
